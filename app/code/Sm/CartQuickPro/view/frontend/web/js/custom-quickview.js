						...
						var _self = $(this),
						  ...
						  if (typeof _id_product !== 'undefined' &&  _id_product !== null){
							   ...
							   _button_container = _self.find(that.options.button_container).length ? _self.find(that.options.button_container) :  _self,
								_button = "<a class='action quickview-handler sm_quickview_handler' title='' href='"+_url+"'><span>"+that.options.label_button+"</span></a>";
			...
			QuickView.prototype.requestQuickView = function() {
				...
				if (_handler.length){
					$('body').off('click', '.quickview-handler').on('click', '.quickview-handler', function(e){
                        ...
                        $(".smcqp-modal-popup .modal-inner-wrap").css({"top":"-125%","transition":"top 2s ease-in-out;"});
						...
						ifr.on('load', function(){
                            ...
                            $('#loading-icon').hide();
							$(".smcqp-modal-popup .modal-inner-wrap").css("top","0px");
