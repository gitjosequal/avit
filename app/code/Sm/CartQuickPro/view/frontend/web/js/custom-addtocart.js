	...
	'domReady!'
		...
		$.widget('cartQuickPro.ajaxCart',{
			options: {
				...
				removeCartItemMessage: $.mage.__('Are you sure you would like to remove this item from the shopping cart?'),
			...
			_initAjaxCart: function () {
				...
				$('body').off('click sumit', _options.addToCartSelector).on('click', _options.addToCartSelector , function (e) {
					e.preventDefault();
					var _that = $(this), _form = $(this).parents('form') , _isCheck = _form.length ? true : false, _isValid = true , _params = '', _oldAction = '', _isAddToCart = false;
                    
                    if(_that.is('.action.tocart')) { _isAddToCart = true; }
                    if(_that.attr('id')=='product-updatecart-button') { _isAddToCart = false; }
					...
					if (_isCheck && !_form.is(_options.formWishlist) && !_that.is('.action.action-delete') && !_that.is('.action.action-edit')){
						...
						var _dataPost = (typeof _that.attr('data-post') !== 'undefined') ? $.parseJSON(_that.attr('data-post')) : null;
						 if (_dataPost) {
                            ...
                            if(_that.is('.action.action-delete'))
                            {
                                confirm({
                                    modalClass: 'smcqp-confirm',
                                    content: _options.removeCartItemMessage ,
                                    actions: {
                                        confirm: function () {
                                            _self._sendAjax(_oldAction, _params, _isAddToCart );
                                        },
            ...
            _afterAjax: function (json, isaddtocart){
                ...
                if( isaddtocart ) {
                   _self._loadSliderCart(); 
                }
            },
            _loadSliderCart: function(){
                var ajax_url = window.checkout.baseUrl+'filterproducts/cart/slider/'; 
                var _self = this;
                $.ajax({
                    type: 'POST',
                    url: ajax_url,
                    data: {
                        related_products_cart: 1,
                        title_module: 'Customers also bought'
                    },
                    success: function (data) {
                        if (data.items_markup != '' && $(data.items_markup).length) {
                            if($('#slider_minicart').length){
                                $('#slider_minicart').parent().html(data.items_markup);
                            } else {
                                var build_slider_html = '<div data-owl="owl-slider" data-autoplay="false" data-nav="false" data-dots="true" data-screen0="1" data-screen481="1" data-screen768="1" data-screen992="1" data-screen1200="1" data-screen1441="1" data-screen1681="1" data-screen1920="1" data-margin="10" data-autoplayhoverpause="true" data-loop="false" data-center="false" data-stagepadding="0" data-mousedrag="true" data-touchdrag="true">';
                                build_slider_html +=data.items_markup;
                                build_slider_html +='</div>';
                                $('#minicart-content-wrapper').append(build_slider_html);
                            }
                             _self._addCaroselSliderCart();
                             _self._hideLoader();
                            $(_self.options.idContainer).customModal("closeModal");
                            
                            $(document).trigger("afterAjaxProductsLoaded");
                            $(document).trigger("afterAjaxLazyLoad");
                            if(!$('html').hasClass('show-minicart-drop')){
                                $('.action.showcart').trigger('click');
                            }  
                        } else {                            $(_self.options.idContainer).customModal("closeModal");                            $('.action.showcart').trigger('click');                        }
                    },
                    dataType: 'json'
                });
            },
            _addCaroselSliderCart(){
				...
				$.ajax({
					...
					if (typeof json.url !== 'undefined' && json.url !== null){
                        ...
                        if (typeof json.success !== 'undefined' && json.success == true) {
                            _self._afterAjax(json, isaddtocart);
                            if (isaddtocart){
