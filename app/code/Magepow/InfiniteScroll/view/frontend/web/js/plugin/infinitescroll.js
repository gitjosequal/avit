    ...
    var IAS = function($element, options) {
        ...
        this.getNextUrl = function(container) {
            ...
            if(typeof(next_url) != 'undefined') {
                next_url += next_url.includes('?') ? '&ajax=1' : '?ajaxs=1';
            } else {
                next_url = '';
            }
        ...
        this.load = function(url, callback, delay) {
            if(this.itemsContainerSelector=='.main'){
               var self = this,
                $itemContainer,
                items = [],
                timeStart = +new Date(),
                ...
                $.ajax({
                    ...
                    if (response.success) {
                        ...
                        var $itemContainer = res.products_list;
                        
                        $($itemContainer).find(self.itemSelector).each(function() {
                            items.push(this);
                        });
                        ...
                        if (callback) {
                            ...
                            if (timeDiff < delay) {
                                setTimeout(function() {
                                    callback.call(self, $itemContainer, items);
                                }, delay - timeDiff);
                            } else {
                                callback.call(self, $itemContainer, items);
                            }
        ...
        this.render = function(items, callback) {
            ...
            promise.done(function() {
                ...
                $(items).fadeIn(400, function() {
                    ...
                    $(document).trigger("afterAjaxProductsLoaded");
                    $(document).trigger("afterAjaxLazyLoad");
                    $('body').trigger('updateAutoClickAttributes');
                    if (callback) {
                        callback();
                    }
                });
...
var IASHistoryExtension = function (options) {
    ...
    this.getPrevUrl = function (container) {
        ...
        if(typeof(prev_url) != 'undefined') {
            prev_url += prev_url.includes('?') ? '&ajax=1' : '?ajax=1';
        } else {
            prev_url = '';
        }
        return prev_url;
